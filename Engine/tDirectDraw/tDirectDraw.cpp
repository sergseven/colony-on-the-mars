...
BYTE tDirectDraw::CreateOffScreenSurface(UINT width,UINT height)
{
	...
	result=lpDD->CreateSurface(&ddSurfaceDesc,
       ...
       BYTE* bmpbuf, int w, int h )
{
 ...
 // ѕроверить, совпадает ли формат файла с форматом поверхности
 // ≈сли совпадает, пересылку можно ускорить функцией memcpy()
...
BOOL tDirectDraw::CopyBMP(LPDIRECTDRAWSURFACE *lpSurf,HBITMAP hbm)
{
	...
	SelectObject (hdcImage, hbm);
	GetObject (hbm, sizeof(bm), &bm);
	// данна€ функци€ дает нам DC св€занный с поверхностью
	//     после чего мы можем работать с ним как с обычным Windows DC
	(*lpSurf)->GetDC (&hdc);
	// "блиттим" нашу картинку на поверхность
	//     после этого на поверхности у нас будет хранитьс€ нужна€ нам картинка
...
BOOL tDirectDraw::CopyDibToSurface(BYTE surface,char szBitmap[],
...
//	return TRUE;
//}
inline void /*BOOL*/ tDirectDraw::BlitImage(int x,int y,
											BYTE &surfaceNum,
											RECT* rect)
{
	transFlag=DDBLTFAST_NOCOLORKEY;
	if(m_colorKeySet[surfaceNum])transFlag=DDBLTFAST_SRCCOLORKEY;
	/*exitLoop=FALSE;
	do
	{
		result=*/
	lpDDSBack->BltFast(x,y,lpDDSOffSurface[surfaceNum],rect,
		                          transFlag|DDBLTFAST_WAIT);
...
inline void /*BOOL*/ tDirectDraw::FlipSurfaces()
{
	/*exitLoop=FALSE;
	do
	{
...
void tDirectDraw::ClearScreen(void)
{
	DDBLTFX bltfx;
	memset(&bltfx,0,sizeof(bltfx));
	bltfx.dwSize = sizeof(bltfx);
	// в данной строке мы указываем цвет, которым нам нужно "закрасить" экран
	//     0 - это черный цвет
	bltfx.dwFillColor = 0;
	for (int i = 0;i<=1;i++)
	{
		lpDDSBack->Blt(0, 0, 0, DDBLT_COLORFILL | DDBLT_WAIT, &bltfx);
		// мы используем флаг DDBLT_COLORFILL чтобы показать, что мы хотим заполнить
		//    требуемую поверхность одним цветом
		FlipSurfaces();
	}
}

BOOL tDirectDraw::SetTransparentColor(BYTE surface,int r,int g,int b)
{
	DDCOLORKEY ddck;
	ddck.dwColorSpaceLowValue = DDConvertRGB(lpDDSOffSurface[surface], 
						(COLORREF) RGB (r, g, b));
	ddck.dwColorSpaceHighValue = ddck.dwColorSpaceLowValue;
	result = lpDDSOffSurface[surface]->SetColorKey(
    DDCKEY_SRCBLT, &ddck);
	/*
...
BOOL tDirectDraw::SetTransparentColor(BYTE surface,int low,int high)
{
	DDCOLORKEY ddck;
	ddck.dwColorSpaceLowValue = low;
	ddck.dwColorSpaceHighValue = high;
	result=lpDDSOffSurface[surface]->SetColorKey(DDCKEY_SRCBLT,&ddck);
/*	DDCOLORKEY ddColorKey;
	ddColorKey.dwColorSpaceLowValue =lowColor;
	ddColorKey.dwColorSpaceHighValue = hiColor;
	result = lpDDSOffSurface[surface]->SetColorKey(
...
// функци€ дл€ перевода значени€ цвета дл€ поверхности pdds из RGB в DWORD
DWORD tDirectDraw::DDConvertRGB(LPDIRECTDRAWSURFACE pdds,COLORREF rgb)
{
	COLORREF rgbT;
	HDC hdc;
...
void tDirectDraw::HLine(int x,int y,int xend,DWORD c){
	DDBLTFX ddbltfxmem;					//local ddbltfx object
	RECT dest;					//create a rectangle for the 
	memset(&ddbltfxmem,0,sizeof(ddbltfxmem));
	ddbltfxmem.dwSize=sizeof(ddbltfxmem); 
	ddbltfxmem.dwFillColor=c;	//set the fill color
	dest.left=x;				//set the destination
	dest.top=y;					//coordinates
	dest.right=xend;
	dest.bottom=y+1;
	lpDDSBack->Blt(&dest,		//pointer to destination RECT
...
void tDirectDraw::VLine(int x,int y,int yend,DWORD c){
	DDBLTFX ddbltfxmem;					//local ddbltfx object
	RECT dest;					//create a rectangle for the 
	memset(&ddbltfxmem,0,sizeof(ddbltfxmem));
	ddbltfxmem.dwSize=sizeof(ddbltfxmem); 
...
void tDirectDraw::DEBar(int x,int y,int lx,int ly,DWORD c){//DrawEmptyBar
	HLine(x,y,x+lx+1,c);
	HLine(x,y+ly,x+lx+1,c);
	VLine(x,y+1,y+ly,c);
	VLine(x+lx,y+1,y+ly,c);
}