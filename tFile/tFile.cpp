    ...
    if(m_file)
        if(fclose(m_file))
            throw tError(__FILE__,__LINE__,"tFile::~tFile",
                         "Can't close file! / file - %s / flags - 0x%x",m_fileName,m_fileFlags);
		m_fileName.erase();
}
...
void tFile::OpenFile(const char* name,int flags)
{
    ...
    if(!m_file)
        throw tError(__FILE__,__LINE__,"tFile::OpenFile",
                     "Can't open file! / file - %s / flags - 0x%x",m_fileName.c_str(),m_fileFlags);
...
void tFile::CloseFile(void)
{
    if(m_file)
    {
		tFile::LogMessage("-Beginning to close file BUG");
        if(fclose(m_file))
